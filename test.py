...
async def mt5_status(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text(
            f"✅ MT5 Connection Active\n\n"
            f"💰 Balance: {balance:.2f}\n"
            f"⏰ Server Time: {server_time}\n"
            f"🔗 Account: {MT5_CREDENTIALS['login']}"
        )
    except Exception as e:
        await update.message.reply_text(f"❌ MT5 Error: {str(e)}")
    finally:
        mt5.shutdown()
...
async def show_account(update_or_query, context: ContextTypes.DEFAULT_TYPE):
    ...
    response = (
        ...
        f"💸 TOTAL WITHDRAWS: *{total_withdrawals} USDT*\n\n"   
    ...
    try:
        ...
        response += f"\n🔹 MT5 Live Balance: *{mt5_balance:.2f} USD*"
    except Exception as e:
        logger.error(f"MT5 balance check failed: {e}")
        response += "\n🔹 MT5 Balance: Unavailable"
    finally:
        mt5.shutdown()
