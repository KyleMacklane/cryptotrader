...
BOT_USERNAME: Final = "Unclehardscalping_bot"
COMMUNITY_LINK = "https://t.me/unclehardbotgroup"
ADMIN_IDS = [8027620965]
#5079683472
#8027620965
...
async def handle_referral_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    success, referrer_telegram_id, is_new_user = account_manager.add_user_if_not_exists(user.id, "main", str(user.id), referral_id=ref_id)

    # Send welcome/start message
    await start(update, context)

    # Notify referrer 
    if success and referrer_telegram_id and is_new_user:
        try:
            await context.bot.send_message(
